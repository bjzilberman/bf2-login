...
exports.challengeResponse = function ( nickname, password, challenge1, challenge2 ) {
...
exports.sessionKey = function ( nickname ) {
  var length = nickname.length;
  var session = index = 0;
  nickname = Buffer.from( nickname );
  while (length != 0) {
    session = crcTable[((nickname[index] ^ session) & 0xff) % 256] ^ (session >>= 8);
    index++; length--;
  }
  return session;
};
...
// Send a tcp message
exports.sendMsg = function ( socket, data ) {
  var message = '\\';
  for(var key in data) {
...
// Handle a game response
exports.respByKey = function ( response, getKey = '' ) {
  buildResp = {};
  response = response.toString().split('\\');
  if(response[0] == '') {
    buildResp.action = response[1];
    response.splice(0,3);
  ...
  while (i < response.length) {
    if(response[i] == 'final') {
      buildResp[response[i]] = 'true';
    } else {
      buildResp[response[i]] = response[i+1];
    }
    i = i+2;
  }
  console.log(buildResp);
  if(getKey != '') {
    return buildResp[getKey];
  } else {
    return buildResp;
  }
};

exports.generateSig = function () {
  var s = '';
  var length = 32;
  ...
  {
      --length;
      s += chars[Math.floor(Math.random() * 14)];
  }
  return s;
}
